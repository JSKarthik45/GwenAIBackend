...
def _make_github_safe_name(name: str) -> str:
    """Convert any string into a GitHub-safe repository name."""
    safe = re.sub(r'[^a-zA-Z0-9_-]', '-', name)
    safe = re.sub(r'-+', '-', safe).strip('-')
    return safe.lower() or "gwen-mvp"


def _create_project_github_repo(
    user_id: str,
    access_token: str,
    project_name: str,
    project_id: str,
) -> dict:
    """Create a fresh private repository per MVP generation."""
    service = GitHubRepoService(client_id=os.getenv("GITHUB_CLIENT_ID") or GitHubRepoService.DEFAULT_CLIENT_ID)
    safe_name = _make_github_safe_name(project_name)
    short_id = project_id.split("-")[0]
    repo_name = f"gwen-mvp-{safe_name}-{short_id}"

    logger.info(f"[github] Creating repository: {repo_name} for user {user_id}")
    repo_info = service.create_repository(
        access_token,
        repo_name=repo_name,
        description=f"React Native MVP for {project_name} generated by Gwen AI",
    )
    return repo_info
...
def _maybe_push_generated_mvp_to_github(
    ...
    try:
        repo_info = _create_project_github_repo(user_id, token, project_name, project_id)
        ...
        async with generation_lock:
            ...
            try:
                ...
                def run_crew_sync():
                    ...
                    result = run_generator(content_prompt=prompt, upload_snack=False)
...
async def github_device_auth_status(request: Request):
    """Poll the GitHub OAuth device flow and return the current status to the frontend without blocking."""
    payload = await _read_json_body(request)
    user_id = str(payload.get("user_id") or payload.get("userId") or "").strip()
    ...
    session = None
    if user_id:
        session = github_auth_sessions.get(user_id)
    if not session and device_code:
        # Fallback to search by device_code
        for sess in github_auth_sessions.values():
            if sess.get("device_code") == device_code:
                session = sess
                user_id = sess.get("user_id")
                break

    if not session:
        return {"status": "error", "message": "session not found for user_id or device_code"}

    resolved_device_code = session["device_code"]
    ...
    try:
        # Check status once using the non-blocking check_device_code_status run in a thread
        res = await asyncio.to_thread(service.check_device_code_status, resolved_device_code)
        
        error_code = res.get("error")
        if not error_code and "access_token" in res:
            ...
            return {"status": "pending", "message": "Authorization is still pending (slow down requested)."}
        elif error_code == "expired_token":
            session["status"] = "expired"
            return {"status": "expired", "message": "GitHub device authorization expired."}
        elif error_code:
            return {"status": "error", "message": f"GitHub authorization failed: {error_code}"}
        else:
            return {"status": "pending", "message": "Authorization is still pending."}
            
    except Exception as exc:
        msg = str(exc)
        logger.warning(f"[github-device-status] error checking status for device_code={resolved_device_code}: {msg}")
